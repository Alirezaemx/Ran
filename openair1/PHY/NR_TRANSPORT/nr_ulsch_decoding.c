...
// [hna] Perform nr_segmenation with input and output set to NULL to calculate only (B, C, K, Z, F)
  ...
  for (r=0; r<harq_process->C; r++) {
    ...
    r_offset += E;
    offset += (Kr_bytes - (harq_process->F>>3) - ((harq_process->C>1)?3:0));
    int latseq_len = (Kr_bytes - (harq_process->F>>3) - ((harq_process->C>1)?3:0));
    LATSEQ_P("U mac.decoded--mac.demuxed", "len%d::frame%d.slot%d.ulsch_id%d.harqpid%d.codeblocksegment%d", latseq_len, frame, nr_tti_rx, ULSCH_id, harq_pid, r);
    //////////////////////////////////////////////////////////////////////////////////////////
  }
  } 
  return 1;
