  ...
  for (int aarx = 0; aarx < ue->frame_parms.nb_antennas_rx; aarx++) {
    ...
    if (config_type == NFAPI_NR_DMRS_TYPE1 && ue->chest_freq == 0) {
      ...
      for (int pilot_cnt = 0; pilot_cnt < 6 * nb_rb_pdsch; pilot_cnt++) {
        if (pilot_cnt % 2 == 0) {
          ch = c16maddShift(*pil, *rxF, (c16_t){0, 0}, 15);
          pil++;
    ...
    // pdsch_dmrs_type2  |dmrs_r,dmrs_l,0,0,0,0,dmrs_r,dmrs_l,0,0,0,0|
      ...
      c16_t ch_l = {0};
      c16_t ch_r = {0};
      for (int pilot_cnt = 0; pilot_cnt < 4 * nb_rb_pdsch; pilot_cnt++) {
        if (pilot_cnt % 2 == 0) {
          ch_l = c16maddShift(*pil, *rxF, (c16_t){0, 0}, 15);
          ...
          ch_r = c16maddShift(*pil, *rxF, (c16_t){0, 0}, 15);
